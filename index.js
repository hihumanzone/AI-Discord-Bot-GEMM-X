...
client.on('interactionCreate', async (interaction) => {
  try {
    const blockedUsers = JSON.parse(fs.readFileSync('import dotenv from 'dotenv';
dotenv.config();
import {
  Client,
  GatewayIntentBits,
...
async function handleButtonInteraction(interaction) {
  ...
  const buttonHandlers = {
    ...
    'change-text-model': changeTextModel,
...
async function handleSelectMenuInteraction(interaction) {
  ...
  const selectMenuHandlers = {
    ...
    'select-text-model': handleTextSelectModel,
  };
      ...
      if (attempts === 0 || stopGeneration) {
        if (!stopGeneration) {
          if (SEND_RETRY_ERRORS_TO_DISCORD) {
            const embed = new EmbedBuilder()
              .setColor(0xFF0000)
              .setTitle(`Generation Failure - ${selectedModel}`)
            ...
            const simpleErrorEmbed = new EmbedBuilder()
              .setColor(0xFF0000)
              .setTitle(`Bot Overloaded - ${selectedModel}`)
              .setDescription('Something seems off. Try clearing your conversation history, or the model might be overloaded! :(');
